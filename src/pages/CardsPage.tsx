            ...
            <div className="w-full max-w-2xl">
              ...
              >
                <div className={`grid transition-transform duration-500 transform-style-3d ${isFlipped ? 'rotate-y-180' : ''}`}>
                  {/* Front */}
                  <div className="col-start-1 row-start-1 backface-hidden glass rounded-3xl p-4 md:p-8 shadow-2xl">
                    {/* Category and Tags */}
                    <div className="mb-4 md:mb-6">
                      <div className="flex items-center gap-2 mb-2 md:mb-3">
                        <span className="px-3 py-1 bg-purple-600/30 text-purple-300 rounded-full text-xs md:text-sm font-medium">
                      ...
                      {currentCard.tags && currentCard.tags.length > 0 && (
                        <div className="flex gap-2 flex-wrap">
                          {currentCard.tags.map(tagId => {
                            ...
                            return (
                              <span key={tagId} className="px-2 md:px-3 py-1 md:py-1.5 bg-blue-600/30 text-blue-300 rounded-full text-xs md:text-sm font-medium border border-blue-500/30">
                                {tagName}
                              </span>
                            );
                          })}
                    ...
                    {currentCard.imageUrl && (
                      <div className="mb-4 md:mb-6 bg-gray-900 rounded-xl overflow-hidden">
                        <img src={currentCard.imageUrl} alt="" className="w-full h-32 md:h-48 object-contain" />
                      </div>
                    )}

                    <div className="flex flex-col items-center justify-center min-h-[150px] md:min-h-[200px] py-4 md:py-8">
                    ...
                    <div className="text-center text-gray-500 text-xs md:text-sm mt-2 md:mt-4">
                      {isFlipped ? '' : 'Нажмите, чтобы перевернуть'}
                    </div>
                  </div>

                  {/* Back */}
                  <div className="col-start-1 row-start-1 backface-hidden rotate-y-180 glass rounded-3xl p-4 md:p-8 shadow-2xl overflow-y-auto">
                    {currentCard.imageUrl && (
                      <div className="mb-4 md:mb-6 bg-gray-900 rounded-xl overflow-hidden">
                        <img src={currentCard.imageUrl} alt="" className="w-full h-32 md:h-48 object-contain" />
                      </div>
                    )}
                    <h3 className="text-lg md:text-xl font-bold text-white mb-4 md:mb-6 text-center">Полная информация</h3>
                      ...
                      {fields.map(field => {
                        ...
                        return (
                          <div key={field.id} className="border-b border-gray-700 pb-2 md:pb-3">
                            ...
                            <div className="text-sm md:text-base text-white leading-relaxed">{value}</div>
                          </div>
                        );
                      })}
                    </div>
                    <div className="text-center text-gray-500 text-xs md:text-sm mt-4 md:mt-6">
              ...
              {currentCard && user && (
                <div className="absolute top-4 right-4 z-10">
                  <div className="glass rounded-full px-3 md:px-4 py-1.5 md:py-2 flex items-center gap-1 md:gap-2">
              ...
              {/* Buttons - Now part of flow, not absolute */}
              <div className="flex flex-col items-center gap-3 md:gap-4 mt-4">
                {/* Study mode controls */}
                {isFlipped && (
                  <div className="flex gap-2 md:gap-4 animate-fade-in w-full justify-center">
                    <button
                      onClick={(e) => { e.stopPropagation(); markStudied(false); }}
                      className="flex-1 max-w-[140px] px-3 md:px-6 py-2 md:py-3 bg-gradient-to-r from-red-600 to-pink-600 hover:from-red-700 hover:to-pink-700 text-white rounded-xl transition font-medium text-xs md:text-sm"
                    >
                      ❌ Неправильно
                    </button>
                    <button
                      onClick={(e) => { e.stopPropagation(); markStudied(true); }}
                      className="flex-1 max-w-[140px] px-3 md:px-6 py-2 md:py-3 bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-700 hover:to-emerald-700 text-white rounded-xl transition font-medium text-xs md:text-sm"
                    >
                      ✅ Правильно
                    </button>
                  </div>
